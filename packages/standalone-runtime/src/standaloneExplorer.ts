...
export function createStandaloneExplorerBridge(
  ...
  function sortLabItemsByLabel(items: ExplorerTreeItem[]): ExplorerTreeItem[] {
    return [...items].sort((left, right) =>
      String(left.label).localeCompare(String(right.label)),
    );
  }

  function buildRunningLabItems(
    filterText: string,
    files: TopologyFileEntry[],
  ): ExplorerTreeItem[] {
    pruneEndpointHealthCache(options.getEndpoints());
    const items = [...collectRunningLabItemsByEndpoint(files).values()].flat();
    return filterTreeItems(sortLabItemsByLabel(items), filterText);
  }

  function buildLocalLabItems(
    filterText: string,
    files: TopologyFileEntry[],
  ): ExplorerTreeItem[] {
    const items = [...collectLocalLabItemsByEndpoint(files).values()].flat();
    return filterTreeItems(sortLabItemsByLabel(items), filterText);
  }
  ...
  function transformSnapshotForEndpointRoots(
    ...
    // Running Labs and Undeployed Local Labs render as their own collapsible,
    // resizable sections (VS Code style), so the snapshot passes through
    // unchanged. We only observe whether a native File Explorer section exists
    // to keep expanded-state persistence keyed correctly.
  ...
  async function buildExplorerProviders(
    ...
    const localItems = buildLocalLabItems(filterText, files);
