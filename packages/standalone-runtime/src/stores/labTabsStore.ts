...
// Only one lab or file is open at a time: `tabs` holds at most one entry and
// opening a different item replaces whatever was open.
export const useLabTabsStore = create<LabTabsStoreState>((set, get) => ({
  ...
  openOrFocusTab: (tab) => {
    ...
    if (current && current.id === tab.id) {
      ...
      const merged: LabTab = keepDirtyFileState
        ...
        : { ...current, ...tab };
      set({ activeTabId: merged.id, tabs: [merged] });
      return { alreadyOpen: true, tab: merged };
    }

    set({ activeTabId: tab.id, tabs: [tab] });
    return { alreadyOpen: false, tab };
  },
  ...
  closeTabsByEndpoint: (endpointId) => {
    const state = get();
    const removed = state.tabs.filter((tab) => tab.endpointId === endpointId);
    if (removed.length === 0) {
      return {
        nextActiveTabId: state.activeTabId,
        removedCount: 0,
    ...
    const removedWasActive =
      state.activeTabId !== null && removedIds.includes(state.activeTabId);
    set({ activeTabId: null, tabs: [] });
