...
import { StandaloneSettingsProvider } from "./components/standaloneSettingsContext";
import { standaloneSettingsSections } from "./components/standaloneSettingsSections";
...
// While a file is open the topology canvas is still mounted underneath the
// editor, so hide the floating TopoViewer navbar but leave the rail/context
// panel (Labs, File Explorer) usable.
...
// Only one item is open at a time, so opening a new item replaces the current
// one. Guard against silently discarding unsaved edits in the open file.
async function confirmReplaceCurrentTab(nextTabId: string): Promise<boolean> {
  const current = useLabTabsStore.getState().tabs[0];
  if (!current || current.id === nextTabId) {
    return true;
  }
  return confirmCloseLabTab(current.id);
}
...
async function openTopologyInTab(
  ...
  if (!(await confirmReplaceCurrentTab(resolvedTab.id))) {
    return;
  }
...
async function openWorkspaceFileInTab(document: {
  content: string;
  endpointId: string;
  path: string;
  title: string;
  ...
  if (!(await confirmReplaceCurrentTab(tab.id))) {
    return;
  }
  const openResult = useLabTabsStore.getState().openOrFocusTab(tab);
  useLabTabsStore.getState().setActiveTab(openResult.tab.id);
  // Only one document is open at a time: opening a file replaces the topology,
  // so tear down the active topology (hides the TopoViewer navbar/palette).
  await queueTopologyTabActivation(async () => {
...
function setupStandaloneUiHost(): void {
  ...
  standaloneRuntime = createClabUiRuntime({
    ...
    customSettingsSections: standaloneSettingsSections(),
  });
}
...
function StandaloneApp() {
  ...
  const runtimeWithColorScheme = useMemo(
    () =>
      standaloneRuntime
        ? {
            ...standaloneRuntime,
