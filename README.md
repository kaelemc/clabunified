...
This monorepo contains the Containerlab web and desktop applications together
with the independently packable `@srl-labs/clab-ui` package.

```text
apps/web                 Browser host
apps/desktop             Electron host
packages/clab-ui         Shared Mantine UI package
packages/app-*           Application contracts and server
packages/standalone-*    Standalone browser runtime
```

Install once at the repository root. `npm run dev` starts the browser-only
frontend with `packages/clab-ui/src` aliased directly into Vite. It does not
start the API-facing Node server, so changes in either the app or UI hot-reload
...
`npm run build` creates the browser-only frontend. The API-backed web and
desktop builds remain available through `npm run build:all`, and the API-backed
development server through `npm run dev:web:server`.

Use `npm run typecheck` and `npm run test:unit` for the full workspace.
`npm run pack:clab-ui` builds and creates an installable
`srl-labs-clab-ui-<version>.tgz` while keeping the package's existing npm name
and subpath exports.
...
The private app workspaces share the root app version. The publishable
`@srl-labs/clab-ui` workspace is versioned independently.
...
The default development command consumes `clab-ui` source directly and
hot-reloads changes without a separate build:

```bash
npm run dev
```

To test the built package boundary instead:

```bash
npm run build:clab-ui
npm run dev:web:package
```

Run `npm run pack:clab-ui` to create the tarball consumed by repositories
